...
class AcrobotCostParams:
    stage_cost_x: float = 0.1 #* 1000
    stage_cost_u: float = 0.01 #* 1000
...
N_mpc = 40  # MPC horizon; horizon length is related to how fast the system is going to converge;for some disturbances we should have a fast convergence and short horizon
...
for k in tqdm(range(myT)):
    ...
    # ax[i].plot(x_mpc[6,:,i]
    x0 = dynamics(x_mpc[k,0,:], u_mpc[k,0,:]+ (np.random.rand(1)-0.5)*4, 0) #
    # x0 += (np.random.rand(4)-0.5)*0.1
...
filename = f'trajax_ilqr_mpc_disturb_cost1x'
outpath = f'./{filename}/'
